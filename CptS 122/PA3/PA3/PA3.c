...
Node * makeNode(char artist[], char title[], char song[], char genre[], int minutes, int seconds, int played, int rating)
{
	...
	if (played < 0 || (rating > 5 || rating < 1)) {
		printf("Invalid fields of entry\n");
		return;
	}
...
void playSongs(Node *pList, char keyArtist[], char keySong[]) {
	Node *pTemp = NULL;
	while (pList != NULL) {
		if (strcmp(pList->song.artist, keyArtist) == 0 && strcmp(pList->song.song, keySong) == 0) {
			system("cls");
	...
	while (pTemp != NULL) {
		system("cls");
		printf("::PLAYING::\n");
		printf("\tArtist: %s\n\tAlbum: %s\n\tSong: %s\n\tGenre: %s\n\tLength: %d:%d\n\tTimes played: %d\n\tRating: %d\n", pTemp->song.artist, pTemp->song.album, pTemp->song.song, pTemp->song.genre, pTemp->song.length.minutes, pTemp->song.length.seconds, pTemp->song.played, pTemp->song.rating);
		Sleep(10000);
		pTemp = pTemp->pNext;
	}
	
}
//Array shuffle from https://bits.mdminhazulhaque.io/c/shuffle-items-of-array-in-c.html
...
void playOneSong(Node *pList){}

void shuffleSongs(Node *pList, int order[]) {
	Node *pTemp = NULL;
	pTemp = pList;
	while (pTemp != NULL){
		int count = 1;
...
void artistSort(Node *pList) {
	...
	Node *pCur = NULL;
	int temp;
	for (pTemp = pList; pTemp->pNext != NULL; pTemp = pTemp->pNext) {
		for (pCur = pTemp->pNext; pCur != NULL; pCur = pCur->pNext) {
			if (strcmp(pTemp->song.artist, pCur->song.artist) > 0) {
